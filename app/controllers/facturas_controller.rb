  ...
  def facturar_venta
    if request.post?
      ...
      if params[:commit] == "Cancelar"
        ...
        #begin 
          certificado_bucket = bucket.file current_user.negocio.datos_fiscales_negocio.path_cer
          ...
          #llave = CFDI::Certificado.new open(url_llave)
          # Esta se convierte de un archivo .key con:
          # openssl pkcs8 -inform DER -in someKey.key -passin pass:somePassword -out key.pem
          llave_bucket = bucket.file current_user.negocio.datos_fiscales_negocio.path_key
          url_llave = llave_bucket.signed_url expires: 600
          password_llave = current_user.negocio.datos_fiscales_negocio.password
...
=end
  #========================================================================================================================
          ...
          consecutivo = 0
          consecutivo = current_user.sucursal.facturas.where(tipo_factura: "fv").last.consecutivo if current_user.sucursal.facturas.where(tipo_factura: "fv").last
          consecutivo += 1

          clave_sucursal = current_user.sucursal.clave
        ...
        factura = CFDI::Comprobante.new({
          ...
          #La moneda por default es MXN
          formaPago: forma_pago_f.cve_forma_pagoSAT,
          #condicionesDePago: 'Sera marcada como pagada en cuanto el receptor haya cubierto el pago.',
          metodoDePago: metodo_pago_f,
